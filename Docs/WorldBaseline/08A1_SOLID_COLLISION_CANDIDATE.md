...
Follow-up 9 августа 2026: это историческое ограничение снято для 32
подтверждённой ручной двери. Project-owned runtime теперь добавляет створкам
движущуюся solid-коллизию и отдельную зону ручки; исходные 79 строк по-прежнему
не превращаются в статические блокеры. Служебные open/closed-пары, статичные
фасады, бытовые приборы и двери транспорта остаются за пределами generic hinge.
Направление каждой створки зафиксировано по донорским клипам и не зависит от
стороны игрока. Домашние гаражные ворота открываются удержанием ЛКМ, закрываются
удержанием ПКМ и после отпускания коротко докатываются с затуханием. Обычные
двери работают только по ЛКМ на зоне ручки; `F` их не активирует. Скорость
обычных дверей снижена до 200°/с, гаражных — до 145°/с с разгоном.
Зона наведения на ручку увеличена с 0,14 до 0,24 м и переведена в
нефизический trigger: интеракционный raycast видит только зарегистрированные
`InteractionTargetHost`, а посторонние trigger-объёмы пропускает.
Точный scope, ownership и остаточные ограничения зафиксированы в
`PHASE1_WORLD_DOOR_BINDING_2026-08-09.md`.
