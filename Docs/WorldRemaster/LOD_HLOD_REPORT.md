...
Три hedge instance добавляют три двухступенчатых `LODGroup`; bounded 05A baseline теперь содержит 67 LOD groups. Pier и shoreline пока не имеют отдельных LOD chains/HLOD proxy: их размер и близкая review-дистанция не оправдывают преждевременный proxy без profiler capture. Pop, shadow transition и дальняя shoreline readability остаются manual gates.
