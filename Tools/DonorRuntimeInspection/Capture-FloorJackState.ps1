...
$payloadSource = Join-Path $PSScriptRoot "RuntimeFloorJackProbe.cs"
$injectorProject = Join-Path $PSScriptRoot "MonoManagedInjector\MonoManagedInjector.csproj"
$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$captureRoot = Join-Path $configuration.DonorStagingDirectory "runtime-inspection\floor-jack\$timestamp"
$payloadPath = Join-Path $captureRoot "MSC.DonorRuntimeFloorJackInspection.V2.dll"
$injectorOutput = Join-Path $captureRoot "injector"
$intermediateOutput = Join-Path $captureRoot "obj\"
$dumpPath = Join-Path ([IO.Path]::GetTempPath()) "msc-runtime-floor-jack-dump-v2.txt"
...
{
    throw "Runtime floor-jack probe compilation failed with exit code $LASTEXITCODE."
}
...
& dotnet $injector $process.Id $monoPath $payloadPath `
    "MSC.DonorRuntimeInspection" "RuntimeFloorJackProbeV2" "Install"
if ($LASTEXITCODE -ne 0)
{
...
{
    throw "The main-thread runtime floor-jack probe produced no dump within 20 seconds."
}

$capturePath = Join-Path $captureRoot "floor-jack-runtime-dump.txt"
Copy-Item -LiteralPath $dumpPath -Destination $capturePath
$firstLine = Get-Content -LiteralPath $capturePath -TotalCount 1
if ($firstLine -ne "MSC_DONOR_RUNTIME_FLOOR_JACK_DUMP_V2")
{
    Get-Content -LiteralPath $capturePath -Raw | Write-Error
    throw "Runtime floor-jack probe returned an error payload."
}
