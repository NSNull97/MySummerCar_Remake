...
{
    /// <summary>
    /// Reference-counted visual ownership marker. It hides matching donor
    /// renderers while deliberately leaving their original colliders active.
    /// </summary>
    [DisallowMultipleComponent]
    public sealed class DonorWorldLegacyRendererOverride : MonoBehaviour
    {
        ...
        private void RegisterKeys()
        {
            ...
            for (int index = 0; index < configured.Length; index++)
            {
                ...
                DonorWorldLegacyReplacementRegistry.Active?
                    .SetProductionRendererOverrideActive(key, true);
            }
        }
        ...
        private void UnregisterKeys()
        {
            foreach (string key in registeredKeys)
            {
                    ...
                    count <= 1)
                {
                    ...
                    DonorWorldLegacyReplacementRegistry.Active?
                        .SetProductionRendererOverrideActive(key, false);
                }
                else
                {
                    ActiveKeyReferences[key] = count - 1;
        ...
        private static void ReapplyActiveOverrides()
        {
            ...
            foreach (KeyValuePair<string, int> pair in ActiveKeyReferences)
                if (pair.Value > 0)
                    registry.SetProductionRendererOverrideActive(pair.Key, true);
        }
